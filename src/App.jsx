...
const CATEGORIES = ["All", "Clothing", "Jewelry", "Necklace", "Bracelet", "Anklet", "Chain", "Earring", "Scarf", "Ring", "Tops", "Dress", "Skirt", "Pant", "Belts", "Accessories", "Bags", "Shoes", "Shawls", "Other"];
...
const RC_CATEGORIES = ["Tops", "Dress", "Skirt", "Pant", "Bags", "Shawls", "Other", "Belts"];
...
const CATEGORY_CODES = {
  ...
  Tops: "T", Dress: "D", Skirt: "I", Pant: "P", Belts: "Y",
};
...
function computeStandardLayout(product, wMm, hMm, opts) {
  ...
  if (showSKU)   items.push({ type:"text", key:"sku",   pref:2.0, min:1.5 });
  ...
  for (const i of textItems) i.fontMm = useFloor ? i.min : i.min + (i.pref - i.min) * scale;
...
function computeJewelryLayout(product, wMm, hMm, opts) {
  const { showBarcode=true, barcodeScale=2 } = opts || {};
  const warnings = [];
  ...
  return {
    barcode,
    skuFontMm:   Math.max(hMm * 0.34, 1.5),
    nameFontMm:  Math.max(hMm * 0.36, 1.7),
    priceFontMm: Math.max(hMm * 0.50, 2.2),
    bizFontMm:   Math.max(hMm * 0.32 * 0.70, 1.5),
...
function drawStandardLabel(ctx, product, W, H, wMm, hMm, opts) {
  ...
  layout.items.forEach((item, idx) => {
    if (item.type === "barcode") {
      ...
      ctx.fillStyle = item.key==="biz" ? bizColor : item.key==="price" ? bizColor : item.key==="sku" ? "#444" : "#111";
...
function drawJewelryLabel(ctx, product, W, H, wMm, hMm, opts) {
  ...
  const pxPerMm = W / wMm;
  const bizColor = (product.business || "Blingshop") === "RC Boutique" ? RC_DARK : GOLD_DARK;
  const FRONT_W = W * 0.42, FOLD_X = W * 0.42, FOLD_W = W * 0.16, BACK_X = W * 0.58, BACK_W = W * 0.42;
  ...
  if (showBarcode && layout.barcode) {
    ...
    if (showSKU) {
      ...
      ctx.fillStyle = "#222"; ctx.font = `${sFsPx}px monospace`;
      ctx.textAlign = "center"; ctx.textBaseline = "alphabetic";
  ...
  // Fold divider
  ctx.fillStyle = "#F0F0F0"; ctx.fillRect(FOLD_X, 0, FOLD_W, H);
  ctx.save();
  ctx.strokeStyle = "#BDBDBD"; ctx.setLineDash([H * 0.14, H * 0.09]); ctx.lineWidth = 0.8;
  ctx.strokeRect(FOLD_X + 1, 1, FOLD_W - 2, H - 2);
  ctx.restore();
  ctx.save();
  ctx.translate(FOLD_X + FOLD_W / 2, H / 2); ctx.rotate(-Math.PI / 2);
  ctx.fillStyle = "#ADADAD"; ctx.font = `bold ${Math.max(H * 0.22, 6)}px Arial`;
  ...
  if (showPrice) {
    ...
    ctx.fillStyle = bizColor; ctx.font = `bold ${pFsPx}px Arial`;
    ctx.textAlign = "center"; ctx.textBaseline = "alphabetic";
    // Clamp the baseline so descenders never fall off the bottom edge, regardless of font size.
    const baselineY = Math.min(H - pFsPx * 0.22, H * 0.92);
...
function generateZPLJewelry(product, wMm, hMm, qty = 1, opts) {
  ...
  if (showPrice) {
    const yTop = Math.max(2, H - pFs - 2); // keep the text box clear of the bottom edge regardless of font size
...
function BulkAddModal({ existingProducts, pendingProducts, onSave, onClose, blingshopThbRate, setBlingshopThbRate, rcThbRate, setRcThbRate }) {
  ...
  const handleSave = async (closeAfter) => {
    ...
    try {
      ...
      const item = {
        id: generateId(), business, category, sku, name: `${sku} ${price}`,
          ...
          <>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10, maxHeight:480, overflowY:"auto" }}>
              {pendingProducts.map(item => (
                <div key={item.id} style={{ display:"flex", gap:10, alignItems:"flex-start", background:LIGHT, border:`1px solid ${BORDER}`, borderRadius:10, padding:10 }}>
                  <input type="checkbox" checked={!!selected[item.id]} onChange={() => setSelected(s => ({ ...s, [item.id]: !s[item.id] }))} style={{ width:15, height:15, marginTop:10, accentColor:GOLD_DARK, cursor:"pointer" }} />
                  ...
                  <div style={{ flex:1, minWidth:0, display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                    ...
                    <div>
                      ...
                      <select value={getVal(item,"category")} onChange={e=>setVal(item.id,"category",e.target.value)} style={{ ...inp, marginTop:2, fontSize:12, padding:"6px 8px" }}>
                        {CATEGORIES.slice(1).map(c => <option key={c}>{c}</option>)}
                      </select>
                    </div>
                    <div>
...
function LabelPrintModal({ product, zebraIP, onClose }) {
  ...
  const [opts, setOpts]           = useState({
    ...
    barcodeScale: 2, printDPI: 300, vAlign: "center",
  });
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:20 }}>
        ...
        <StatCard label="Total Revenue" value={`LKR ${totRev.toLocaleString()}`} sub="All time" accent="#5C4A1E" />
...
function RefundModal({ sale, refunds, onClose, onRefund }) {
  ...
  const doProcess = async (type) => {
    ...
    await onRefund({ items, type, refundAmt, notes,
        ...
        {refundableItems.length === 0 ? (
          ...
          <div style={{ marginTop:12 }}>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Notes (optional)"
              rows={2} style={{ width:"100%", padding:"8px 10px", border:`1px solid ${BORDER}`, borderRadius:8,
          ...
          <div style={{ display:"flex", gap:10, marginTop:14 }}>
            <button onClick={() => doProcess("refund")} disabled={busy || !Object.keys(selected).length}
              style={{ flex:1, padding:"11px", background:"#DC2626", color:WHITE, border:"none", borderRadius:8,
            ...
            <button onClick={() => doProcess("exchange")} disabled={busy || !Object.keys(selected).length}
              style={{ flex:1, padding:"11px", background:GOLD_DARK, color:WHITE, border:"none", borderRadius:8,
                cursor:"pointer", fontWeight:700, fontSize:14, opacity: (!Object.keys(selected).length || busy) ? 0.5 : 1 }}>
